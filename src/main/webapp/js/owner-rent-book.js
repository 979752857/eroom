...
function getDatasList(index){
	...
	_tabIndex = index;
	if(_tabIndex == 1){
		url = myUtil.BASE + "/roombook/getFdRentBookApply";
	}else if(_tabIndex == 2){
		url = myUtil.BASE + "/roombook/getFdRentBookFinish";
	}else if(_tabIndex == 3){
		url = myUtil.BASE + "/roombook/getFdRentBookAll";
	}
 ...
 * 授权预约看房
 */
function agreeBook(bookId){
	var url = myUtil.BASE + "/roombook/agreeBook";
 ...
 * 取消预约看房
 */
function refuseBook(bookId){
	var url = myUtil.BASE + "/roombook/refuseBook";
	var date = {bookId : bookId};
	myUtil.ajax.axs(url,date,function(result){
		if(!result){
			myUtil.toast('服务器繁忙请重试');
...
function getListSuccess(result){
	...
	if(datas&&datas.length>0){
		$.each(datas,function(index,item){
			...
			html += '<a href="room-detail.html?rentId='+item.rentId+'"><div class="tu clearfloat"><span></span><img src="'+myUtil.getSystemCfgTitle("EROOM", "IMAGES")+myUtil.getSystemCfgTitle("EROOM", "IMAGES_ROOM")+item.bedroomImageUrl+'"/>';
			...
			if(item.applyState == '00'){
				html += '<div class="topsche-top entrust box-s clearfloat"><a href="javascript:agreeBook('+item.bookId+');" class="tit fr entrust-btn">同意授权</a>';
				html += '<a href="javascript:refuseBook('+item.bookId+');" class="tit fr entrust-btn">拒绝授权 </a>';
			}
			html += '</div></div></div>';
		});
	}else{
